...
class Goods extends BaseController
{
        ...
        if (request()->isAjax()) {
            ...
            if ($state_type == 2) {
                $child_menu_list = array(
                    ...
                    array(
                        'url' => "goods/erpImport?state_type=2",
                        'menu_name' => "库存同步记录",
                        'active' => 1
                    ),
                );
            ...
            return view($this->style . "Goods/goodsImport");
        }
    }
     ...
     * 同步掌秀云商品sku库存
     */
    public function erpStock(){
        $where ['source'] = '掌秀云ERP';
        $goods_id = $this->request->post('goods_id');
        if ($goods_id) {
            $where['goods_id'] = array('=', $goods_id);
        }
        $goods = new  NsGoodsModel();
        $goods_info = $goods->get($where);
        if (empty($goods_info)){
            $this->success('更新完成');
        }else{
            $goods_info->toArray();
            //获取掌秀云erp商品详情，更新sku信息
            hook('zhangxiuyunOneProduct', ['product_id' => $goods_info['external_id']]);
            $where['goods_id'] = array('>', $goods_info['goods_id']);
            $goods_info = $goods->get($where);
            //echo $goods->getlastsql();die;
            $this->success('sku库存同步成功', '', $goods_info);
        }
    }
    ...
    public function goodsImport()
    {
        if (!empty($_FILES)) {
            ...
            $info = $file->move(ROOT_PATH . 'public/excel' . DS . 'uploads');
            if ($info) {
                $file = ROOT_PATH . 'public/excel' . DS . 'uploads/' . $info->getSaveName();
                $file = str_replace('/', '\\', $file);
                $data = import_excel($file);

                $goodsModel = new NsGoodsModel();//商品
                $goods_spec = new NsGoodsSpecModel();//商品规格名称
                $goodsbrand = new NsGoodsBrandModel();//商品品牌
                $goodsCategory = new NsGoodsCategoryModel();//商品分类
                $goods_spec_value = new NsGoodsSpecValueModel();//商品规格值
                $goods_sku = new  NsGoodsSkuModel();//商品sku
                //首先把文件内的信息处理，完成，然后进行批量插入
                $excel_info = array();
                foreach ($data as $key => $val) {
                    $excel_info[$key]['source'] = trim($val[0]);
                    $excel_info[$key]['goods_name'] = trim($val[1]);
                    $excel_info[$key]['code'] = trim($val[2]);
                    $excel_info[$key]['sku_str'] = trim($val[3]);
                    $excel_info[$key]['sku'] = trim($val[4]);
                    $excel_info[$key]['class1'] = trim($val[5]);
                    $excel_info[$key]['class2'] = trim($val[6]);
                    $excel_info[$key]['class3'] = trim($val[7]);
                    $excel_info[$key]['unit'] = trim($val[8]);
                    $excel_info[$key]['brand'] = trim($val[9]);
                    $excel_info[$key]['market_price'] = trim($val[10]);
                    $excel_info[$key]['cost_price'] = trim($val[11]);
                    $excel_info[$key]['price'] = trim($val[12]);
                    $excel_info[$key]['sku_num'] = trim($val[13]);
                }
                $excel_error = array();
                foreach ($excel_info as $key => $value) {
                    ...
                    if ($value['goods_name'] == null) {
                        $error ['goods_name'] = $value['goods_name'];
                    }
                    if ($value['code'] == null) {
                        $error ['code'] = $value['code'];
                    }
                    if ($value['sku_str'] == null && $value['source'] == "代销") {
                        $error ['sku_str'] = $value['sku_str'];
                    }elseif($value['sku_str'] != "" &&$value['sku_str'] != null && $value['source'] == "代销"){
                        if(strstr($value['sku_str'],"："))
                        {
                            $error ['sku_str'] = $value['sku_str'];
                        }
                    }
                    if ($value['sku'] == null && $value['source'] == "代销") {
                        $error ['sku'] = $value['sku'];
                    }
                    if ($value['class1'] == null) {
                        $error ['class1'] = $value['class1'];
                    }
                    if ($value['class2'] == null) {
                        $error ['class2'] = $value['class2'];
                    }
                    if ($value['class3'] == null) {
                        $error ['class3'] = $value['class3'];
                    }
                    if ($value['unit'] == null && $value['source'] == "代销") {
                        $error ['unit'] = $value['unit'];
                    }
                    if ($value['brand'] == null) {
                        $error ['brand'] = $value['brand'];
                    }
                    if ($value['market_price'] == null && $value['source'] == "代销") {
                        $error ['market_price'] = $value['market_price'];
                    }
                    if ($value['cost_price'] == null && $value['source'] == "代销") {
                        $error ['cost_price'] = $value['cost_price'];
                    }
                    if ($value['price'] == null && $value['source'] == "代销") {
                        $error ['price'] = $value['price'];
                    }
                    if ($value['sku_num'] == null && $value['source'] == "代销") {
                        $error ['sku_num'] = $value['sku_num'];
                    }
                    if ($error) { //如果有错误数据
                        $excel_error[$key] = $error;
                    }else{
                        unset($excel_info[$key]);
                    }

                }
                if ($excel_error){
                   echo json_encode(['errCode'=>80001,'list'=>$excel_info,'errList'=>$excel_error],JSON_UNESCAPED_UNICODE);
                   die;
                }
                //如果没有错误信息//销毁变量
                unset($excel_info);
                foreach ($data as $k => $value) {
                    $goods_info = $goodsModel->get(array('goods_name'=>trim($value[1])));
                    if (empty($goods_info)) {//判断商品是否存在
                        $brand_id = 0;
                        if (!empty($value[9])) { //判断商品品牌
                            $res = $goodsbrand->get(['brand_name' => trim($value[9])]);
                            if (empty($res)) {
                                $brand['shop_id'] = 0;
                                $brand['brand_name'] = trim($value[9]);
                                $brand_id = $goodsbrand->save($brand,false);
                            } else {
                                $res->toArray();
                        ...
                        $category_id1 = 0;
                        if (!empty($value[5])) { //判断商品分类
                            $Category_res1 = $goodsCategory->get(['category_name' => trim($value[5])]);
                            if (empty($Category_res1)) {
                                $category = array();
                                $category[0]['is_visible'] = 1;
                                $category[0]['level'] = 1;
                                $category[0]['category_name'] = trim($value[5]);
                                $category_id1 = $goodsCategory->saveAll($category,false);
                                $category_id1 = $category_id1[0]['category_id'];
                            } else {
                                $Category_res1->toArray();
                                $category_id1 = $Category_res1['category_id'];
                            }
                        }
                        $category_id2 = 0;
                        if (!empty($value[6])) { //判断商品分类
                            $Category_res2 = $goodsCategory->get(['category_name' => trim($value[6])]);
                            if (empty($Category_res2)) {
                                $category = array();
                                $category[0]['pid'] = $category_id1;
                                $category[0]['is_visible'] = 1;
                                $category[0]['level'] = 2;
                                $category[0]['category_name'] = trim($value[6]);
                                $Category_res2 = $goodsCategory->saveAll($category,false);
                                $category_id2 = $Category_res2[0]['category_id'];
                            } else {
                                $Category_res2->toArray();
                                $category_id2 = $Category_res2['category_id'];
                            }
                        }
                        $category_id3 = 0;
                        if (!empty($value[7])) { //判断商品分类
                            $Category_res3 = $goodsCategory->get(['category_name' => trim($value[7])]);
                            if (empty($Category_res3)) {
                                $category = array();
                                $category[0]['pid'] = $category_id2;
                                $category[0]['is_visible'] = 1;
                                $category[0]['level'] = 3;
                                $category[0]['category_name'] = trim($value[7]);
                                $Category_res3 = $goodsCategory->saveAll($category,false);
                                $category_id3 = $Category_res3[0]['category_id'];
                            } else {
                                $Category_res3->toArray();
                                $category_id3 = $Category_res3['category_id'];
                            }
                        }
                        $goods [0]['shop_id'] = 0;
                        $goods [0]['state'] = 0;
                        $goods [0]['source'] = trim($value[0]);
                        $goods [0]['goods_name'] = trim($value[1]);
                        $goods [0]['code'] = trim($value[2]);
                        $goods [0]['category_id'] = $category_id3;
                        $goods [0]['category_id_1'] = $category_id1;
                        $goods [0]['category_id_2'] = $category_id2;
                        $goods [0]['category_id_3'] = $category_id3;
                        $goods [0]['goods_unit'] = trim($value[8]);
                        $goods [0]['brand_id'] = $brand_id;
                        $goods [0]['market_price'] = trim($value[10]);
                        $goods [0]['cost_price'] = trim($value[11]);
                        $goods [0]['price'] = trim($value[12]);
                        $goods [0]['stock'] = trim($value[13]);
                        $goods_id_arr = $goodsModel->saveAll($goods);
                    ...
                    if (!empty($value[3])) {
                        ...
                        $spec_val_arr = explode('，', $value[3]);
                        foreach ($spec_val_arr as $key => $val) {
                            //多重规格
                            $spec_val = explode(':', $val);
                            $spec_info = $goods_spec->get(['spec_name' => trim($spec_val[0])]);
                            if (empty($spec_info)) { //规格名称不存在
                                $spec_name['spec_name'] = trim($spec_val[0]);
                                $spec_name['is_visible'] = 1;
                                $spec_name['sort'] = 0;
                                $spec_name['show_type'] = 1;
                                $spec_name['create_time'] = time();
                                $spec_name['is_screen'] = 1;
                                $spec_id = $goods_spec->save($spec_name,false);//添加规格名称
                            ...
                            if (count($goods_spec_format)!=count($spec_val_arr)) {
                                $goods_spec_format[] = array(
                                    'spec_name' => $spec_name['spec_name'],
                                    'spec_id' => $spec_id,
                                );
                            }
                            $sku_name [] = $spec_name['spec_name'];
                            $spec_value = $goods_spec_value->get(['spec_value_name' => trim($spec_val[1]), 'spec_id' => $spec_id]);
                            if (empty($spec_value)) { //判断规格值是否存在
                                $spec_value_name['spec_id'] = $spec_id;
                                $spec_value_name['spec_value_name'] = trim($spec_val[1]);
                                ...
                                $goods_spec_format[$key]['value'][] = array(
                                ...
                                $spec_value_name['spec_value_name'] = $spec_value['spec_value_name'];
                            }
                            if (!empty($goods_spec_format[$key]['value'])) {
                                $bbb =array(
                                    'spec_value_name' => $spec_value_name['spec_value_name'],
                                    'spec_name' => $spec_name['spec_name'],
                                    'spec_id' => $spec_id,
                                ...
                                if (!in_array($bbb, $goods_spec_format[$key]['value'])) {
                                    $goods_spec_format[$key]['value'][] = array(
                                        'spec_value_name' => $spec_value_name['spec_value_name'],
                                        'spec_name' => $spec_name['spec_name'],
                                        'spec_id' => $spec_id,
                            ...
                            $sku_arr[] = $spec_id . ':' . $spec_value_id;
                        }
                        $goods_sku->where(['attr_value_items' => implode(';', $sku_arr), 'goods_id' => $goods_id])->delete();//删除sku信息
                        $sku['sku_name'] = implode(' ', $sku_name);
                        $sku['attr_value_items'] = implode(';', $sku_arr);
                        $sku['attr_value_items_format'] = implode(';', $sku_arr);
                        $sku['market_price'] = trim($value[10]);
                        $sku['cost_price'] = trim($value[11]);
                        $sku['price'] = trim($value[12]);
                        $sku['code'] = trim($value[4]);
                        $sku['stock'] = trim($value[13]);
                        $sku['create_date'] = time();
                        $sku_id = $goods_sku->save($sku, false);
                        $goodssku['goods_spec_format'] = json_encode($goods_spec_format, JSON_UNESCAPED_UNICODE);
                        //更新商品sku
                        $goodsModel->save($goodssku, ['goods_id' => $goods_id]);
                        unset($goodssku);
                        unset($goods_spec_format);
                        unset($sku_arr);
                        unset($sku_name);
                    }else{ //如果为空，掌秀商品
                        $goods_spec_format = [];
                ...
                //删除文件
                @unlink($file);
                echo json_encode(['errCode'=>10000,'list'=>'','errList'=>''],JSON_UNESCAPED_UNICODE);
                die;
            } else {
                // 上传失败获取错误信息
                echo $file->getError();
    ...
    public function zhangxiuErp()
    {
        if (request()->isAjax()) {
            ...
            if (!empty($goods_info)) {
                $goods_info->toArray();
                //获取掌秀云erp商品详情，更新sku信息
                hook('zhangxiuyunProductDetail', ['product_code' => $goods_info['code']]);
                $where['goods_id'] = array('>', $goods_info['goods_id']);
                $goods_info = $goods->get($where);
                $this->success('sku同步成功', '', $goods_info);
            }else{
                $this->success('更新完成', '', '');
            }
        } else {
     ...
     * 导出表头
     */
    public function exportGoodsTitle()
    {
        $xlsName = "商品导入表";
        $xlsCell = array(
            array('source', '来源'),
            array('goods_name', '商品名称(不能为空)'),
            array('code', '商品编码(不能为空)'),
            array('sku_str', '商品规格'),
            array('sku', '规格对应的sku编码'),
            array('class1', '一级分类'),
            array('class2', '二级分类'),
            array('class3', '三级分类'),
            array('unit', '计量单位'),
